...
#import data
data = np.genfromtxt('AtoE.csv',  delimiter=',')#enter file name. I put data in a certain format
data = np.delete(data, (0), axis=0)#remove labels columb
actual = np.genfromtxt('Actual.csv',  delimiter=',')
actual = np.delete(actual, (0), axis=0)#remove labels columb
actual = actual[:,2] #0 or 1 results col 2, ordered by index

def precision(index, scores, true):#input score array and actual ordered by subj trail, output precision
    ...
    correct = 0
    for i in holdIndex:#looks up the index in the true set and checks to see if correct
...
#get rankings
def ranking(scores, col):# input ordered scores and col # of score, output rank col same order as input
    hold_same = []
    rank = []
    count = 0
...
"""
maybe split into train and test
Are we assuming this whole this is the training data and we want the highest accuracy in the training data?
 or should we split into train and test and see how it does in training data?
"""
...
def mixGroupRank(ranks):#input ranks, output average of lowest rank in every combo of A to E
...
def mixGroupScore(scores):#input scores, output average of highest score in every combo of A to E
...
for x in range(0,6):
    temp = []
    for i in range(0,6):
        temp.append(cogdiversity(pre.minmax_scale(orderedL1[x]),pre.minmax_scale(orderedL1[i])))
    cogdiv.append(temp)

avgcogdiv = []
for i in cogdiv:
    avgcogdiv.append(sum(i)/len(i))
w1CogDiv = avgcogdiv/sum(avgcogdiv)  # enter the average cog div normalized to be a weight
